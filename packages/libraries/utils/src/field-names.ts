...
if (import.meta.vitest) {
    ...
    describe("`IsValidField`", () => {
        it("gives back exact string on strings of valid variable names", () => {
            expectTypeOf<ValidField<"someVar">>().toEqualTypeOf<"someVar">();
            expectTypeOf<
                ValidField<"__weird_var">
            >().toEqualTypeOf<"__weird_var">();
            expectTypeOf<ValidField<"someVar2">>().toEqualTypeOf<"someVar2">();
        });

        it("gives a `TypeErrorMessage` on strings of invalid variable names", () => {
            expectTypeOf<
                ValidField<"2noLettersAtStart">
            >().toMatchTypeOf<TypeErrorMessage>();
            expectTypeOf<
